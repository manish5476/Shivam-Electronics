...
export class SignupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    ); // Apply the custom validator at the form group level
  }
    ...
    this.auth.signUp(signupDetails).subscribe({
      next: (response: any) => {
        ...
        if (response?.token) {
          ...
          this.errorMessage =
            response?.message ||
            'Signup process completed, but could not log you in. Please try logging in manually.';
