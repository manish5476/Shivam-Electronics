...
export class CustomerListComponent implements OnInit {
    ...
    private totalCount = 0;   // 🔹 Backend total
    private pageSize = 15;    // 🔹 Default limit
    data: any[] = [];
    customer: any;
    column: any = [];
    ...
    getData(isReset: boolean = false) {
        ...
        const filterParams = {
            ...
            limit: this.customerFilter.limit || this.pageSize,
        };
        ...
        this.CustomerService.getAllCustomerData(filterParams).subscribe(
            (res: any) => {
                ...
                if (this.gridApi && newData.length) {
    ...
    validateCustomer(): boolean {
        return !!(this.customer?.fullname &&
            this.customer?.email &&
            /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(this.customer.email));
    }

    saveCustomer() {
        if (this.validateCustomer()) {
            this.customer.mobileNumber = Number(this.customer.phoneNumbers[0].number);
    ...
    getColumn(): void {
        this.column = [
            ...
            { tooltipField: 'email', field: 'email', headerName: 'Email', sortable: true, filter: true, resizable: true },
            { tooltipField: 'fullname', field: 'fullname', headerName: 'Full Name', sortable: true, filter: true, resizable: true },
            ...
            { tooltipField: 'totalPurchasedAmount', field: 'totalPurchasedAmount', headerName: 'Total Purchased Amount', sortable: true, filter: true, resizable: true },
            {
            ...
            { tooltipField: '__v', field: '__v', headerName: 'Version', sortable: true, filter: true, resizable: true },
...
//         this.autopopulatedata()
//         this.getColumn();
//         this.getData(true); // Initial fetch
//     }
